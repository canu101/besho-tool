    ...
    try:
        ...
        fb_dtsg = await page.evaluate('''() => {
            try {
                ...
                // 2. data-store
                const script = Array.from(document.querySelectorAll('script')).find(s => s.textContent.includes('DTSGInitialData'));
                if(script){
                ...
                if(window.requireLazy){
                    try {
                        const dtsg = requireLazy('DTSGInitialData')?.token;
                        if(dtsg) return dtsg;
                    } catch(e){}
                }
                // 4. localStorage
                return localStorage.getItem('dtsg') || null;
            } catch(e){ return null; }
        }''')
